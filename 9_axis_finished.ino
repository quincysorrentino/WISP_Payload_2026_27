...
#include <math.h>
#include <Servo.h>
#include <SPI.h>  // Added for SD communication
#include <SD.h>   // Added for SD functions

Servo pitchServo;
Servo rollServo;
File myFile;      // Object for the SD file
const int chipSelect = 4; // Current pin selected could needs changing
...
void setup() {
  ...
  if (!SD.begin(chipSelect)) {
    ...
    while (1); // Stop if SD card is not found
  }
  ...
  if (myFile) {
    myFile.println("q0,q1,q2,q3,Roll,Pitch,AccCal,GyrCal,MagCal,SysCal,TimeMS");
    myFile.close();
  }
...
void loop() {
  ...
  if (myFile) {
    // Record Quaternions
    myFile.print(q0); myFile.print(",");
    myFile.print(q1); myFile.print(",");
    myFile.print(q2); myFile.print(",");
    myFile.print(q3); myFile.print(",");
    
    // Record Euler Angles
    myFile.print(rollActual); myFile.print(",");
    myFile.print(pitchActual); myFile.print(",");
    
    // Record Calibration and Time
    myFile.print(accel); myFile.print(",");
    myFile.print(gyro); myFile.print(",");
    myFile.print(mg); myFile.print(",");
    myFile.print(system); myFile.print(",");
    myFile.println(millis());
    
    myFile.close(); // Saves the data to the card
  }
